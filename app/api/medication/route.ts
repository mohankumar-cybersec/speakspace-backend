...
export async function POST(request: NextRequest) {
  try {
    ...
    // 3. Handle SpeakSpace Parser (Hackathon Requirements)
    // SpeakSpace sends: { "prompt": "{\"type\": ...}", "note_id": "..." }
    // We need to unwrap the 'prompt' string into our actual JSON object.
    let payloadType = body.type;
    let payloadData = body.data;

    if (body.prompt) {
      try {
        console.log('Unwrapping SpeakSpace Prompt:', body.prompt);
        // The prompt field contains the JSON string generated by the AI
        const parsedPrompt = JSON.parse(body.prompt);
        payloadType = parsedPrompt.type;
        payloadData = parsedPrompt.data;
      } catch (e) {
        console.error('Failed to parse SpeakSpace prompt string:', e);
        // Fallback: If prompt isn't JSON, maybe treat it as raw text input?
        // For now, let's assume the Prompt Template worked correctly.
      }
    }
